import { v4 as uuidv4 } from "uuid"; 

function chillHop() {
    return [

        {
            name:"La Tricolor",
            artist: "LaTricolor",
            cover: "https://img.freepik.com/vector-gratis/silueta-aparato-radio_23-2147501018.jpg",
            id: uuidv4(),
            color: ["#F24933", "#F9B437"],
            audio: "https://27143.live.streamtheworld.com/KMXXFMAAC.aac?dist=onlineradiobox",
            active: true,
        },
        {
            name:"José 97.5",
            artist: "José97.5",
            cover: "https://img.freepik.com/vector-gratis/radio-plana-retro-ilustrada_23-2148812864.jpg",
            id: uuidv4(),
            color: ["#D7B0D5", "#CBDFE0"],
            audio: "https://23263.live.streamtheworld.com/KLYYFMAAC.aac?dist=onlineradiobox",
            active: false,
        },
        {
            name:"Radio Lobo MX",
            artist: "RadioLoboMX",
            cover: "https://img.freepik.com/vector-gratis/aire-letrero-neon-marco_23-2148818543.jpg",
            id: uuidv4(),
            color: ["#1D2731", "#F17E76"],
            audio: "https://sp2.servidorrprivado.com/9302/stream",
            active: false,
        },
        {
            name:"La Ke Buena 93.7",
            artist: "LaKeBuena93.7",
            cover: "https://img.freepik.com/vector-gratis/fondo-musica_53876-90528.jpg",
            id: uuidv4(),
            color: ["#008781", "#cfc08c"],
            audio: "https://radiovision.mi-radio.stream:11006/stream",
            active: false,
        },
        {
            name:"La Naranjera de Sibers",
            artist: "LaNaranjeradeSibers",
            cover: "https://img.freepik.com/vector-gratis/dia-mundial-radio-diseno-plano_23-2148802732.jpg",
            id: uuidv4(),
            color: ["#845648","#FDFBDF"],
            audio: "https://radiostreamingaac.com:8010/stream",
            active: false,
        },
        {
            name:"Tejano Music Network",
            artist: "TejanoMusicNetwork",
            cover: "https://img.freepik.com/vector-gratis/marco-neon-vivo-aire_23-2148758076.jpg",
            id: uuidv4(),
            color: ["#578CCE", "#E9D6D0"],
            audio: "https://us9.streamingpulse.com/ssl/tejanomusicnetwork",
            active: false,
        },https://radioenhd.com/8088/;
        {
            name:"El Jefe 95.4 HD",
            artist: "ElJefe95.4HD",
            cover: "https://img.freepik.com/vector-gratis/texto-plano-dia-mundial-radio_23-2148825802.jpg",
            id: uuidv4(),
            color: ["#191516", "#82698E"],
            audio: "https://s01.digitalserver.org:7088/;stream.nsv",
            active: false,
        },
        {
            name:"La más Grupera",
            artist: "LamásGrupera",
            cover: "https://img.freepik.com/vector-gratis/ilustracion-realista-auriculares-inalambricos_1284-20763.jpg",
            id: uuidv4(),
            color: ["#7BC8DD", "#D64A59"],
            audio: "https://radioenhd.com/8088/;",
            active: false,
        },
        {
            name:"La Mejor",
            artist: "LaMejor",
            cover: "https://img.freepik.com/vector-gratis/fondo-retro-dia-mundial-radio_23-2147594195.jpg",
            id: uuidv4(),
            color: ["#FDE1A9", "#A46874"],
            audio: "https://26513.live.streamtheworld.com/XERCFM_SC?dist=onlineradiobox",
            active: false,
        },
    ];
}

export default chillHop;


